### H2O Tour Chicago - Training ###
# Binary Classification Problem: Predict whether a loan is good or bad

library(h2o)
h2o.init(nthreads = -1,   #how many cores to run in the machine. -1 means "all the cores". 
        max_mem_size = "8G")  # max memory to allocate to H2O. allocate 
                              # x2-x5 the size of your dataset.E.g 1GB data set, allocate 4GB of memory. 
...
#[6] "annual_inc"            "purpose"               "addr_state"            "dti"                   "delinq_2yrs"          
#[11] "revol_util"            "total_acc"             "bad_loan"              "longest_credit_length" "verification_status"  
...
glm_fit1 <- h2o.glm(x = x, 
                    ...
                    model_id = "glm_fit1",   #give an id to the model to look it later in flow.
...
glm_fit2 <- h2o.glm(x = x, 
                    ...
                    training_frame = train,
                    model_id = "glm_fit2",
                    validation_frame = valid,  #we had the validation frame because of the param tuning. we didn't use it before.
                    family = "binomial",
                    lambda_search = TRUE)   #controls the amount of Regularization. Controls overfit/how loss the model. 
...
# Print model performance. To see the Test set metrics. 
glm_perf1
glm_perf2
...
rf_fit3 <- h2o.randomForest(x = x,
                            ...
                            model_id = "rf_fit3",
                            seed = 1,
                            nfolds = 5)   #add this for K-Fold CV. 

# To evaluate the cross-validated AUC, do the following:
h2o.auc(rf_fit3, xval = TRUE)  # 0.6572501
...
# Retreive test set AUC
h2o.auc(gbm_perf1)  # 0.716462
h2o.auc(gbm_perf2)  # 0.8045909
h2o.auc(gbm_perf3)  # 0.779619
